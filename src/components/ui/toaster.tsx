...
// Import the useToast hook with a dynamic import to avoid circular dependencies
let useToast: () => { toasts: Array<{ id: string; title?: string; description?: string; action?: React.ReactNode }> };

try {
  // Try to import useToast dynamically
  useToast = require('./use-toast').useToast;
} catch (e) {
  // Fallback if use-toast is not available
  useToast = () => ({ toasts: [] });
}
...
// Extend the existing ToastProps type
...
export function Toaster() {
  ...
  return (
    <ToastProvider>
      {toasts.map(({ id, title, description, action, ...props }: ToastComponentProps) => (
        <Toast key={id} {...props}>
