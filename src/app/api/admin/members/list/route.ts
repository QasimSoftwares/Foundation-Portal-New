...
/**
 * GET /api/admin/members/list?status=pending|approved|rejected|all&page=1&pageSize=20
 * Returns a normalized list of member records matching the given status.
 * - pending/rejected are sourced from role_requests (request_type = 'member')
 * - approved is sourced from user_roles where is_member = true
 */
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (status === "pending") {
      const r1 = await supabase.rpc("get_pending_member_requests");
      results.push(r1);
    } else if (status === "approved") {
      const r1 = await supabase.rpc("get_approved_members");
      results.push(r1);
    } else if (status === "rejected") {
      const r1 = await supabase.rpc("get_rejected_member_requests");
      results.push(r1);
    } else {
      // 'all' status - get all types
      const r1 = await supabase.rpc("get_pending_member_requests");
      const r2 = await supabase.rpc("get_approved_members");
      const r3 = await supabase.rpc("get_rejected_member_requests");
      results.push(r1, r2, r3);
    }

    const errors = results.map(r => r.error).filter(Boolean);
    if (errors.length > 0) {
      logger.error("Member list RPC error", { errors });
    ...
    logger.error("Unexpected error in member list endpoint", { error: error instanceof Error ? error : new Error(String(error)) });
    return new NextResponse(JSON.stringify({ error: "Internal Server Error" }), { status: 500 });
  }
}
