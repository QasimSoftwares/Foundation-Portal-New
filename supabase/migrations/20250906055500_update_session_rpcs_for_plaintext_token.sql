-- Update session RPCs to be compatible with existing refresh_tokens schema
-- This version writes BOTH token (plaintext) and token_hash (sha256) to accommodate legacy schema

-- Ensure pgcrypto is available
CREATE EXTENSION IF NOT EXISTS pgcrypto WITH SCHEMA public;

-- Backfill token_hash from token where missing
UPDATE public.refresh_tokens
SET token_hash = encode(digest(token, 'sha256'), 'hex')
WHERE token_hash IS NULL AND token IS NOT NULL;

-- Replace create_user_session to also write into token column (plaintext)
...
BEGIN
  ...
  BEGIN
    ...
    INSERT INTO public.refresh_tokens (
      user_id,
      session_id,
      token,           -- legacy plaintext column (NOT NULL)
      token_hash,      -- new hashed column
      expires_at,
      ip_address,
      user_agent
    ) VALUES (
...
-- Replace refresh_user_session to update both token and token_hash
...
BEGIN
  ...
  -- Mark any matching old tokens as revoked
  UPDATE public.refresh_tokens
  SET revoked = true
  ...
  RETURN QUERY SELECT v_token_record.session_id, v_new_token_id;
...
-- Revoke permissions then grant to authenticated role
